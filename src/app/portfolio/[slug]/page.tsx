          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
                ...
                <h2 className="text-2xl font-bold text-white">The Operational Problem</h2>
              ...
              <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
                ...
                <h2 className="text-2xl font-bold text-white">What Was Built</h2>
                <p className="text-slate-300 text-base leading-relaxed">
                  {project.solution}
                </p>
              </div>
                ...
                {project.impactMetrics && project.impactMetrics.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-slate-800">
                    {project.impactMetrics.map((m) => (
                      <div key={m.label} className="p-4 rounded-xl bg-slate-950 border border-slate-800">
                        <span className="block text-2xl font-black font-mono text-emerald-400 mb-1">
                          {m.value}
                        </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {projectTeam.map((person) => (
                      <Link
                        ...
                        className="p-4 rounded-xl bg-slate-900 border border-slate-800 hover:border-red-500/50 transition-all flex items-center gap-4"
                      >
                        <div className="w-10 h-10 rounded-full bg-red-950 text-red-400 font-mono font-bold text-sm flex items-center justify-center border border-red-800">
                          {person.name.split(" ").map((n) => n[0]).join("")}
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-white">{person.name}</h4>
                          <span className="text-xs font-mono text-slate-400">{person.role}</span>
                        </div>
                      </Link>
                    ))}
                  </div>
                    ...
                    {projectPartners.map((partner) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] font-mono text-slate-400">{partner.category}</span>
                        </div>
                        <p className="text-xs text-slate-400">{partner.collaborationFocus}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
                ...
                <div className="space-y-2">
                  {projectCountries.map((c) => (
                    <div key={c.code} className="flex items-center justify-between text-xs text-slate-300">
                ...
                {relatedProjects.map((rp) => (
                  ...
                  >
                    <div className="space-y-3">
                      <span className="text-[10px] font-mono font-bold text-red-400">{rp.category}</span>

...
export function BlogCard({ post }: { post: BlogPost }) {
  return (
    <article className="group rounded-2xl bg-slate-900 border border-slate-800 hover:border-red-500/50 transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-xl">
      <div>
        ...
        <div className="relative w-full aspect-video bg-slate-950 overflow-hidden">
          ...
          <div className="absolute bottom-3 left-3">
            <span className="px-2.5 py-1 rounded text-xs font-mono font-bold bg-slate-950/90 text-red-400 border border-red-800/80 backdrop-blur-sm">
              {post.category}
            </span>
          </div>
        </div>
      ...
      <div className="p-6 pt-0 space-y-4">
        <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full bg-red-950 text-red-400 border border-red-800 font-mono font-bold text-[10px] flex items-center justify-center">
              {post.author.name.split(" ").map((n) => n[0]).join("")}
            </div>
            <span className="text-xs font-mono text-slate-300 font-medium truncate max-w-[140px]">
              {post.author.name}
            </span>
          </div>
